    ...
    <div class="bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-600 animate-gradient-x rounded-xl shadow-xl p-6 mb-8 text-white relative overflow-hidden">
        ...
        <div class="relative animate-fade-in z-10">
            ...
            <div class="mt-4 flex flex-wrap gap-3">
                <a href="{{ route('siswa.dashboard') }}" class="btn-glass flex items-center px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
        ...
        <div class="dashboard-card bg-white rounded-xl shadow-sm p-6 transform transition-all hover:scale-105 relative overflow-hidden">
            ...
            <div class="flex items-start relative">
                ...
                <div class="ml-4 flex-1">
                    ...
                    <p class="card-number floating-element">{{ $exams->count() ?? 0 }}</p>
                    <div class="mt-2">
        ...
        <div class="dashboard-card bg-white rounded-xl shadow-sm p-6 transform transition-all hover:scale-105 relative overflow-hidden">
            ...
            <div class="flex items-start relative">
                ...
                <div class="ml-4 flex-1">
                    ...
                    <p class="card-number floating-element">{{ $exams->where('status', 'active')->where('start_date', '<=', now())->where('end_date', '>=', now())->count() ?? 0 }}</p>
                    <div class="mt-2">
        ...
        <div class="dashboard-card bg-white rounded-xl shadow-sm p-6 transform transition-all hover:scale-105 relative overflow-hidden">
            ...
            <div class="flex items-start relative">
                ...
                <div class="ml-4 flex-1">
                    ...
                    <p class="card-number floating-element">{{ $exams->where('end_date', '<', now())->count() ?? 0 }}</p>
                    <div class="mt-2">
        ...
        <div class="dashboard-card bg-white rounded-xl shadow-sm p-6 transform transition-all hover:scale-105 relative overflow-hidden">
            ...
            <div class="flex items-start relative">
                ...
                <div class="ml-4 flex-1">
                    ...
                    <p class="card-number floating-element">{{ $exams->where('start_date', '>', now())->count() ?? 0 }}</p>
                    <div class="mt-2">                        
                ...
                <div class="space-y-4">
                    @foreach($exams as $index => $exam)
                        <div class="border border-gray-200 
                            ...
                            @elseif($exam->start_date <= now() && $exam->end_date >= now())
                                bg-blue-50 border-blue-200
                            @else
                                bg-orange-50 border-orange-200
                            @endif
                            rounded-lg p-4 transition-all hover:shadow-md hover:-translate-y-1 duration-300" style="--animation-order: {{ $index }}">
                            <div class="flex">
                                <div class="flex-shrink-0">
                                    <span class="inline-flex items-center justify-center h-12 w-12 rounded-xl 
                                        ...
                                        @endif">
                                        <i class="fas fa-{{ $exam->end_date < now() ? 'check-double' : ($exam->start_date <= now() && $exam->end_date >= now() ? 'file-alt' : 'hourglass-start') }} text-xl"></i>
                                    </span>
                                </div>
                                <div class="ml-4 flex-1">
                                    <div class="flex flex-wrap justify-between mb-1">
                                        <h4 class="text-base font-medium text-gray-800 mb-1">{{ $exam->title }}</h4>
                                        <span class="text-sm 
                                            @if($exam->end_date < now())
                                                text-gray-600
                                            @elseif($exam->start_date <= now() && $exam->end_date >= now())
                                    ...
                                    <div class="flex items-center text-sm text-gray-500 mb-3">
                                        ...
                                        <span>{{ $exam->total_questions ?? 'N/A' }} Pertanyaan</span>
                                    </div>
                                    <div class="flex justify-between items-center">
                                        <a href="{{ route('siswa.exams.show', $exam->id) }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-800 inline-flex items-center group">
