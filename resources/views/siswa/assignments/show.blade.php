...
<div class="container mx-auto px-4 py-6">
    ...
    <!-- Back Button -->
    <div class="mb-6">
        <a href="{{ route('siswa.assignments.index') }}" class="text-blue-600 hover:text-blue-900 inline-flex items-center">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden mb-6">
        ...
        <div class="p-5 border-b border-gray-200 flex justify-between items-center
            {{ $assignment->isExpired() ? 'bg-red-50' : 'bg-blue-50' }}">
            <div>
            ...
            <!-- Status Badge -->
            <span class="px-4 py-2 rounded-full {{ $assignment->isExpired() ? 'bg-red-500' : 'bg-blue-500' }} text-white text-sm font-semibold">
                {{ $assignment->isExpired() ? 'Deadline Terlewati' : 'Aktif' }}
            </span>
        </div>
        ...
        <div class="p-5 bg-gray-50">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <div class="bg-white p-3 rounded border border-gray-200 flex items-center">
                        ...
                        <span class="text-gray-800">{{ $assignment->deadline ? $assignment->deadline->format('d M Y H:i') : 'No Deadline Set' }}</span>
                    </div>
                </div>
                <div>
                    ...
                    <div class="bg-white p-3 rounded border border-gray-200">
                        @if($submission)
                            <span class="text-green-600">
                                ...
                                Tugas sudah dikumpulkan pada {{ $submission->created_at->format('d M Y H:i') }}
                            </span>
                        @else
            ...
            <div class="mb-4">
                ...
                <div class="bg-white p-3 rounded border border-gray-200">
                    <a href="{{ asset('storage/' . $assignment->file) }}" target="_blank" class="flex items-center text-blue-500 hover:text-blue-700">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden mb-6">
        <div class="p-5 bg-gradient-to-r from-indigo-50 to-purple-50 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-600 mt-1">
                {{ $submission ? 'Anda sudah mengumpulkan tugas ini. Anda dapat mengupdate pengumpulan tugas jika diperlukan.' 
        ...
        <div class="p-5 bg-green-50 border-b border-gray-200">
            ...
            <div class="flex items-start bg-white p-3 rounded border border-gray-200">
                <div class="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-lg bg-blue-100 text-blue-500">
                    <i class="fas fa-{{ $submission->file_icon ?? 'file' }} text-lg"></i>
                </div>
                <div class="ml-3 flex-1">
                    <h5 class="font-medium text-gray-800">{{ $submission->file_name }}</h5>
                    <div class="flex items-center text-sm text-gray-600 mt-1">
                        <span class="mr-3">{{ $submission->file_size ?? 'Unknown size' }}</span>
                        <span>Diupload: {{ $submission->submitted_at ? $submission->submitted_at->format('d M Y H:i') : 'N/A' }}</span>
                    </div>
                    <div class="mt-2 flex space-x-2">
                        <a href="{{ asset('storage/' . $submission->file_path) }}" target="_blank" class="text-blue-500 hover:text-blue-700">
        ...
        @endif        <!-- Submission Form -->
        <div class="p-5">
            @if(!$assignment->isExpired() || ($submission && $assignment->allow_late_submission))
                <form action="{{ $submission ? route('siswa.assignments.update-submission', $assignment) : route('siswa.assignments.submit', $assignment) }}" method="POST" enctype="multipart/form-data" class="space-y-4" id="submission-form">
                    @csrf
                    <div>
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="3" 
                                ...
                                placeholder="Tambahkan catatan untuk tugas ini...">{{ $submission->notes ?? '' }}</textarea>
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="bg-white p-3 rounded border border-gray-200 flex items-center">
                        ...
                        <span class="text-gray-800">{{ $submission->graded_at ? $submission->graded_at->format('d M Y H:i') : 'Belum dinilai' }}</span>
                    </div>
                </div>
            </div>
        ...
        if (submissionForm) {
            submissionForm.addEventListener('submit', function(e) {
                ...
                if (!e.submittedViaModal) {
                    ...
                    @if(!isset($hoursRemaining) || $hoursRemaining >= 24)
