...
@section('content')
    ...
    <div class="bg-gradient-to-r from-red-500 to-red-600 rounded-xl shadow-lg p-6 mb-6 text-white relative overflow-hidden animate-fade-in">
        ...
        <div class="relative z-10 flex justify-between items-center">
            ...
            <a href="{{ route('guru.announcements.create') }}" class="px-4 py-2 bg-white text-red-600 rounded-lg hover:bg-red-50 transition-colors shadow-md hover:shadow-lg flex items-center gap-2">
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-6">
        <div class="p-4 bg-gray-50 border-b">
            <form action="{{ route('guru.announcements.index') }}" method="GET" class="flex flex-wrap items-end gap-4">
                <div class="flex-grow min-w-[200px]">
                    ...
                    <select name="importance" id="importance" class="w-full rounded-lg border-gray-300 focus:border-red-500 focus:ring focus:ring-red-200 focus:ring-opacity-50" onchange="this.form.submit()">
                        <option value="all" {{ request('importance') == 'all' || !request('importance') ? 'selected' : '' }}>Semua Pengumuman</option>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Cari judul pengumuman..." class="rounded-lg pr-10 border-gray-300 focus:border-red-500 focus:ring focus:ring-red-200 focus:ring-opacity-50 w-full">
                        <button type="submit" class="absolute inset-y-0 right-0 px-3 flex items-center bg-red-500 text-white rounded-r-lg">
                            <i class="fas fa-search"></i>
                        </button>
                    </div>
                </div>
                @if(request('importance') || request('search'))
                    <a href="{{ route('guru.announcements.index') }}" class="px-3 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors flex items-center gap-2">
                ...
                @foreach($announcements as $announcement)
                    <div class="p-6 hover:bg-gray-50 transition-colors {{ $announcement->is_important ? 'bg-red-50' : '' }}">
                        <div class="flex items-start">
                            <div class="bg-{{ $announcement->is_important ? 'red' : 'gray' }}-100 p-3 rounded-lg text-{{ $announcement->is_important ? 'red' : 'gray' }}-600 mr-4 flex-shrink-0">
                                <i class="fas {{ $announcement->is_important ? 'fa-exclamation-circle' : 'fa-bell' }} text-xl"></i>
                            </div>
                            <div class="flex-1">
                                ...
                                <p class="text-gray-600 mt-2 text-sm line-clamp-2">{{ $announcement->content }}</p>
                                ...
                                <div class="mt-4 flex items-center gap-2">
                                    <a href="{{ route('guru.announcements.show', $announcement) }}" class="px-3 py-1 bg-gray-100 text-gray-800 rounded hover:bg-gray-200 text-sm flex items-center gap-1 transition-colors">
                                        <i class="fas fa-eye"></i> Detail
                                    </a>
                                    @if($announcement->author_id == auth()->id())
                                        <a href="{{ route('guru.announcements.edit', $announcement) }}" class="px-3 py-1 bg-blue-100 text-blue-800 rounded hover:bg-blue-200 text-sm flex items-center gap-1 transition-colors">
                                            <i class="fas fa-edit"></i> Edit
                                        </a>
                                        <form action="{{ route('guru.announcements.destroy', $announcement) }}" method="POST" class="inline" onsubmit="event.preventDefault(); showDeleteModal('{{ route('guru.announcements.destroy', $announcement) }}')">
                                            @csrf
                                            @method('DELETE')
        ...
        @else
            <div class="p-8 text-center">
                ...
                <a href="{{ route('guru.announcements.create') }}" class="inline-flex items-center px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">
