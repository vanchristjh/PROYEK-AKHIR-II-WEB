...
@section('content')
        ...
        <div class="relative z-10">
            ...
            <div class="flex flex-wrap gap-4 mt-6">
                <span class="inline-flex items-center px-3 py-1.5 rounded-full bg-blue-500/30 text-blue-50 text-sm backdrop-blur-sm">
                    <i class="fas fa-user-clock mr-2"></i> Guru: {{ Auth::user()->name }}
                </span>
                <span class="inline-flex items-center px-3 py-1.5 rounded-full bg-blue-500/30 text-blue-50 text-sm backdrop-blur-sm">
    ...
    <!-- New Schedule Alert -->
    @if(session('new_schedules_count'))
            ...
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    <p class="text-sm font-medium">
                        {{ session('new_schedules_count') }} jadwal baru telah ditambahkan oleh admin.
            ...
            <div class="text-center mb-6 animate-pulse-light">
                <div class="inline-block rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 p-1 mb-3">
                    <div class="bg-white rounded-lg px-4 py-2">
                        <h4 class="text-2xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">{{ $todayInIndonesian }}, {{ now()->format('d F Y') }}</h4>
                    </div>
                </div>
            ...
            @if(isset($schedulesByDay[$todayInIndonesian]) && count($schedulesByDay[$todayInIndonesian]) > 0)
                <div class="space-y-4">
                    @foreach($schedulesByDay[$todayInIndonesian] as $schedule)
                        ...
                        <div class="flex items-center p-5 rounded-xl border-2 {{ $statusClass }} transition-all duration-300 hover:shadow-md transform hover:-translate-y-1 group">
                            <div class="flex-shrink-0 p-3 rounded-full {{ $statusIconClass }} shadow-sm group-hover:shadow-md transition-all duration-300">
                                <i class="fas {{ $isOngoing ? 'fa-play-circle' : ($isUpcoming ? 'fa-clock' : 'fa-check-circle') }} text-lg"></i>
                            </div>
                            <div class="ml-4 flex-1">
                                <h4 class="text-lg font-semibold text-gray-900">{{ $schedule->subject->name }}</h4>
                                <div class="flex items-center text-sm text-gray-600 mt-1">
                                    <span><i class="fas fa-chalkboard mr-1"></i> {{ $schedule->classroom->name }}</span>
                                    <span class="mx-2">•</span>
                                    <span><i class="far fa-clock mr-1"></i> {{ $schedule->start_time }} - {{ $schedule->end_time }}</span>
                                </div>
                            </div>
                            <div class="ml-4">
                        ...
                        <div class="mb-8">
                            ...
                            <div class="space-y-4">
                                @foreach($daySchedules as $schedule)
                                    ...
                                    @endphp
                                    <div class="p-4 hover:bg-blue-50 transition-colors duration-300 rounded-lg border {{ $isNewSchedule ? 'border-blue-300 animate-pulse-light' : 'border-transparent' }}">
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center">
                                                ...
                                                <div class="ml-4">
                                                    <div class="flex items-center">
                                                        <p class="text-sm font-semibold text-gray-900">{{ $subjectName }}</p>
                                                        @if($isNewSchedule)
                                                            <span class="ml-2 bg-green-100 text-green-800 text-xs px-2 py-0.5 rounded-full">Baru</span>
                                                        @endif
                                                    </div>
                                                    <p class="text-xs text-gray-500">{{ $classroomName }}</p>
                                                </div>
                                            </div>
        ...
        <div class="bg-gradient-to-br from-white to-green-50 rounded-xl shadow-md p-6 border border-green-100 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-3xl font-bold text-gray-800 mt-1">{{ count($totalClasses) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-white to-purple-50 rounded-xl shadow-md p-6 border border-purple-100 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-3xl font-bold text-gray-800 mt-1">{{ count($totalSubjects) }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200 hover:shadow-lg transition-all duration-300">
        ...
        <div class="p-6">
            ...
            <div class="flex flex-wrap gap-4">
                <a href="{{ route('guru.schedule.export.pdf') }}" class="px-5 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors inline-flex items-center shadow-sm hover:shadow-md">
                    <i class="fas fa-file-pdf mr-2"></i> Ekspor PDF
                </a>
                <a href="{{ route('guru.schedule.export.excel') }}" class="px-5 py-2.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors inline-flex items-center shadow-sm hover:shadow-md">
