...
@section('content')
    ...
    <div class="bg-gradient-to-r from-yellow-500 to-amber-600 animate-gradient-x rounded-xl shadow-xl p-6 mb-6 text-white relative overflow-hidden">
        ...
        <div class="relative animate-fade-in z-10">
            ...
            <div class="mt-4 flex flex-wrap gap-3">
                <a href="{{ route('guru.quizzes.create') }}" class="bg-white/15 backdrop-blur-sm hover:bg-white/25 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 flex items-center shadow-md hover:shadow-lg hover:-translate-y-1">
        ...
        <div class="dashboard-card bg-white rounded-xl shadow-sm p-6 transform transition-all hover:scale-105 relative overflow-hidden">
            ...
            <div class="flex items-start relative">
                ...
                <div class="ml-4 flex-1">
                    ...
                    <p class="text-2xl font-bold text-gray-800">{{ $quizzes->count() }}</p>
                    <div class="mt-2">
                        <span class="text-xs text-gray-500">
                            <i class="fas fa-check-circle text-green-500 mr-1"></i> 
                            {{ $activeQuizzes ?? 0 }} Kuis Aktif
                        </span>
                    </div>
                </div>
            </div>
                ...
                <div class="ml-4 flex-1">
                    ...
                    <div class="mt-2">
                        <span class="text-xs text-gray-500">
                            ...
                            Dari {{ $totalParticipants ?? 0 }} Siswa
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="dashboard-card bg-white rounded-xl shadow-sm p-6 transform transition-all hover:scale-105 relative overflow-hidden">
            ...
            <div class="flex items-start relative">
                ...
                <div class="ml-4 flex-1">
                    ...
                    <p class="text-2xl font-bold text-gray-800">{{ $completedQuizzes ?? 0 }}</p>
                    <div class="mt-2">
                        <span class="text-xs text-gray-500">
                            <i class="fas fa-clock text-amber-500 mr-1"></i> 
                            {{ $pendingQuizzes ?? 0 }} Kuis Tertunda
                        </span>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 mb-6 border border-gray-100/50">
        <form action="{{ route('guru.quizzes.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-8">
        @forelse($quizzes as $quiz)
            <div class="bg-white rounded-xl shadow-sm hover:shadow-md transition-all border border-gray-100/60 overflow-hidden group">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex flex-wrap gap-2 mb-3">
                                ...
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-purple-100 text-purple-800">
                                    <i class="fas fa-clock mr-1"></i> {{ $quiz->duration ?? 0 }} menit
                                </span>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-blue-100 text-blue-800">
                                    <i class="fas fa-question mr-1"></i> {{ $quiz->questions_count ?? 0 }} Soal
                                </span>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-green-100 text-green-800">
                                    <i class="fas fa-users mr-1"></i> {{ $quiz->attempts_count ?? 0 }} Mengerjakan
                                </span>
                            </div>
                            ...
                            <div class="flex items-center text-xs text-gray-500">
                                <div class="flex items-center">
                                    ...
                                    <span>Mulai: {{ $quiz->start_time ? \Carbon\Carbon::parse($quiz->start_time)->format('d M Y, H:i') : 'Tidak diatur' }}</span>
                                </div>
                                <div class="w-1 h-1 rounded-full bg-gray-300 mx-2"></div>
                                <div class="flex items-center">
                                    <i class="fas fa-calendar-check mr-1"></i>
                                    <span>Berakhir: {{ $quiz->end_time ? \Carbon\Carbon::parse($quiz->end_time)->format('d M Y, H:i') : 'Tidak diatur' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-gray-50 px-5 py-3 border-t border-gray-100 flex justify-between items-center">
                    <div>
                        <a href="{{ route('guru.quizzes.show', $quiz->id) }}" class="text-sm text-blue-600 hover:text-blue-800 font-medium hover:underline transition-colors">
                            <i class="fas fa-eye mr-1"></i> Detail
                        </a>
                        <a href="{{ route('guru.quizzes.results', $quiz->id) }}" class="ml-3 text-sm text-green-600 hover:text-green-800 font-medium hover:underline transition-colors">
                            <i class="fas fa-chart-bar mr-1"></i> Hasil
                        </a>
                    </div>
                    <div class="flex space-x-2">
                        <a href="{{ route('guru.quizzes.edit', $quiz->id) }}" class="p-2 text-gray-500 hover:text-blue-600 transition-colors">
                            <i class="fas fa-edit"></i>
                        </a>
                        <form action="{{ route('guru.quizzes.destroy', $quiz->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kuis ini?')">
                            @csrf
                            @method('DELETE')
        ...
        @empty
            <div class="lg:col-span-2 bg-white rounded-xl shadow-sm p-8 text-center border border-gray-100/60">
                ...
                <a href="{{ route('guru.quizzes.create') }}" class="inline-flex items-center px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg transition-colors">
                    <i class="fas fa-plus mr-2"></i> Buat Kuis Baru
                </a>
            </div>
        @endforelse
