        ...
        <div class="border-b border-gray-200">
            <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                <button type="button" class="tab-button active border-amber-500 text-amber-600 py-4 px-1 border-b-2 font-medium text-sm" data-tab="submissions-tab">
                    <i class="fas fa-file-alt mr-2"></i> Penilaian Tugas ({{ $submissions->total() }})
                </button>
                <button type="button" class="tab-button border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 py-4 px-1 border-b-2 font-medium text-sm" data-tab="direct-tab">
                    <i class="fas fa-star mr-2"></i> Penilaian Langsung ({{ $directGrades->total() }})
                </button>
            </nav>
        </div>
    </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($submissions as $submission)
                            <tr class="hover:bg-gray-50">
                                        ...
                                        <div class="h-10 w-10 flex-shrink-0">
                                            <div class="h-10 w-10 rounded-full bg-purple-100 flex items-center justify-center">
                                                <span class="text-purple-800 font-medium text-sm">
                                                    {{ substr($submission->student->name ?? 'N/A', 0, 2) }}
                                                </span>
                                            </div>
                                        </div>
                                        <div class="ml-4">
                                            ...
                                            <div class="text-sm text-gray-500">
                                                {{ $submission->student->nis ?? 'N/A' }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900">{{ $submission->assignment->title ?? 'N/A' }}</div>
                                    <div class="text-xs text-gray-500">Dikumpulkan: {{ $submission->created_at->format('d/m/Y H:i') }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900">{{ $submission->assignment->subject->name ?? 'N/A' }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900">{{ $submission->student->classroom->name ?? 'N/A' }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    @if($submission->score !== null)
                                        <span class="text-sm text-gray-900">{{ $submission->score }}/{{ $submission->assignment->max_score }}</span>
                                    @else
                                ...
                                <td class="px-6 py-4 text-right text-sm font-medium space-x-2">                                    <a href="{{ route('guru.submissions.grade', $submission->id) }}" class="text-purple-600 hover:text-purple-900">
    ...
    <div id="direct-tab" class="tab-content hidden">
        <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100/50">
            <div class="flex justify-between items-center p-5 bg-gray-50">
                ...
                <a href="{{ route('guru.grades.create') }}" class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors flex items-center">
                    <i class="fas fa-plus mr-2"></i> Buat Penilaian
                </a>
            </div>
            <div class="overflow-x-auto">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($directGrades as $grade)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="h-10 w-10 flex-shrink-0">
                                            <div class="h-10 w-10 rounded-full bg-purple-100 flex items-center justify-center">
                                                <span class="text-purple-800 font-medium text-sm">
                                                    {{ substr($grade->student->name ?? 'N/A', 0, 2) }}
                                                </span>
                                            </div>
                                        </div>
                                        <div class="ml-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900">{{ $grade->classroom->name ?? 'N/A' }}</div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900">{{ $grade->score }}/{{ $grade->max_score }}</div>
                                    @if($grade->feedback)
                                        <div class="text-xs text-gray-500">{{ Str::limit($grade->feedback, 30) }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900">{{ $grade->created_at->format('d/m/Y') }}</div>
                                    <div class="text-xs text-gray-500">{{ $grade->created_at->format('H:i') }}</div>
                                </td>
                                <td class="px-6 py-4 text-right text-sm font-medium space-x-2">
                                    <a href="{{ route('guru.grades.edit', $grade) }}" class="text-indigo-600 hover:text-indigo-900">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <button onclick="confirmDelete('{{ route('guru.grades.destroy', $grade) }}', '{{ $grade->student->name }}', '{{ ucfirst($grade->type) }}')" class="text-red-600 hover:text-red-900">
