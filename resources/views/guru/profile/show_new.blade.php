        ...
        <div class="relative p-8 overflow-hidden bg-gradient-to-r from-purple-600 to-indigo-600">
            ...
            <div class="relative z-10 grid items-center grid-cols-1 gap-8 md:grid-cols-12">
                ...
                <div class="text-center md:col-span-9 md:text-left">
                    <h1 class="mb-3 text-3xl font-bold text-white md:text-4xl">{{ $user->name }}</h1>
                    <div class="flex flex-wrap justify-center gap-6 md:justify-start text-white/90">
                        @if($user->teacher && $user->teacher->nip)
                        ...
                        @if($user->teacher && $user->teacher->phone)
                            <div class="flex items-center px-4 py-2 space-x-2 rounded-full backdrop-blur-sm bg-white/10">
                                <i class="fas fa-phone"></i>
                                <span>{{ $user->teacher->phone }}</span>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="max-w-4xl mx-auto">
                <div class="flex flex-wrap items-center justify-between gap-4">
                    <div class="flex items-center space-x-3">
                        <span class="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium {{ isset($user->teacher) && isset($user->teacher->profile_completed) && $user->teacher->profile_completed ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800' }} transition-all duration-200 hover:shadow-md">
                            @if(isset($user->teacher) && isset($user->teacher->profile_completed) && $user->teacher->profile_completed)
                                <i class="mr-2 fas fa-check-circle"></i> Profil Lengkap
                            @else
                                <i class="mr-2 fas fa-exclamation-circle"></i> Perlu Dilengkapi
                            @endif
                        </span>
                        @if(!isset($user->teacher) || !($user->teacher->profile_completed ?? false))
                            <a href="{{ route('guru.profile.edit') }}" 
    ...
    <div class="grid grid-cols-1 gap-8 lg:grid-cols-12">
        ...
        <div class="space-y-8 lg:col-span-4">
            ...
            <div class="p-6 transition-all duration-300 bg-white shadow-lg rounded-2xl hover:shadow-xl">
                ...
                <div class="space-y-6">
                    @if($user->teacher && $user->teacher->address)
                        <div class="flex items-start group">
                            ...
                            <div class="ml-3">
                                ...
                                <p class="mt-1 text-gray-600">{{ $user->teacher->address }}</p>
                            </div>
                        </div>
                    @endif
                    @if($user->teacher && $user->teacher->bio)
                        <div class="flex items-start group">
                            ...
                            <div class="ml-3">
                                ...
                                <p class="mt-1 text-gray-600">{{ $user->teacher->bio }}</p>
                            </div>
                        </div>
                    @endif
                </div>
                    ...
                    @if($user->teacher && $user->teacher->phone)
                        <div class="flex items-center group">
                            ...
                            <div class="ml-3">
                                ...
                                <p class="mt-1 text-gray-600">{{ $user->teacher->phone }}</p>
                            </div>
                        </div>
                    @endif
                </div>
                            ...
                            @foreach($todaySchedules as $schedule)
                                <div class="flex items-start p-5 transition-all duration-200 rounded-xl bg-gray-50 hover:bg-purple-50 hover:shadow-md group">
                                    ...
                                    <div class="flex-1 ml-5">
                                        <h4 class="text-lg font-medium text-gray-900">{{ $schedule->subject->name }}</h4>
