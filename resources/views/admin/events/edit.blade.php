...
<div class="bg-white shadow-md rounded-lg p-6">
    <div class="flex items-center mb-6">
        ...
        <h2 class="text-xl font-semibold text-gray-800">Edit Event - {{ $event->title }}</h2>
    </div>
    ...
    <form method="POST" action="{{ route('admin.events.update', $event->id) }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $event->title) }}" class="form-input w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
            </div>
            
            <div>
                <label for="location" class="block text-sm font-medium text-gray-700 mb-1">Lokasi</label>
                <input type="text" name="location" id="location" value="{{ old('location', $event->location) }}" class="form-input w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" name="start_date" id="start_date" 
                    ...
                    class="form-input w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
            </div>
            ...
            <div>
                ...
                <select name="audience" id="audience" class="form-select w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                    <option value="all" {{ (old('audience', $event->audience) == 'all') ? 'selected' : '' }}>Semua</option>
                    <option value="teachers" {{ (old('audience', $event->audience) == 'teachers') ? 'selected' : '' }}>Guru</option>
                    <option value="students" {{ (old('audience', $event->audience) == 'students') ? 'selected' : '' }}>Siswa</option>
                    <option value="admin" {{ (old('audience', $event->audience) == 'admin') ? 'selected' : '' }}>Admin</option>
                </select>
            </div>
            ...
            <div>
                ...
                <div class="flex items-center space-x-4 mt-2">
                    <label class="inline-flex items-center">
                        <input type="radio" name="is_active" value="1" {{ $event->is_active ? 'checked' : '' }} class="form-radio text-blue-600 focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        <span class="ml-2">Aktif</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="radio" name="is_active" value="0" {{ !$event->is_active ? 'checked' : '' }} class="form-radio text-blue-600 focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4" class="form-textarea w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ old('description', $event->description) }}</textarea>
        </div>
        ...
        <div>
            ...
            <div class="mb-3">
                <div class="w-full md:w-1/3 mb-2">
                    <img src="{{ asset('storage/' . $event->image_path) }}" alt="{{ $event->title }}" class="rounded-md max-w-full h-auto shadow">
                </div>
                <p class="text-sm text-gray-600">Gambar saat ini. Unggah yang baru untuk mengganti.</p>
            </div>
            @endif
