<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up()
    {
        Schema::table('assignments', function (Blueprint $table) {
            // Check and rename columns only if they exist
            if (Schema::hasColumn('assignments', 'file_path')) {
                $table->renameColumn('file_path', 'file');
            }
            
            if (Schema::hasColumn('assignments', 'due_date')) {
                $table->renameColumn('due_date', 'deadline');
            }
            
            // Add missing fields if they don't exist
            if (!Schema::hasColumn('assignments', 'is_active')) {
                $table->boolean('is_active')->default(true);
            }
            
            if (!Schema::hasColumn('assignments', 'max_score')) {
                $table->integer('max_score')->nullable();
            }
            
            if (!Schema::hasColumn('assignments', 'allow_late_submission')) {
                $table->boolean('allow_late_submission')->default(false);
            }
            
            if (!Schema::hasColumn('assignments', 'late_submission_penalty')) {
                $table->integer('late_submission_penalty')->default(0);
            }
            
            if (!Schema::hasColumn('assignments', 'visibility')) {
                $table->string('visibility')->default('visible');
            }
        });
    }

    public function down()
    {
        Schema::table('assignments', function (Blueprint $table) {
            // Check and rename columns back only if they exist
            if (Schema::hasColumn('assignments', 'file')) {
                $table->renameColumn('file', 'file_path');
            }
            
            if (Schema::hasColumn('assignments', 'deadline')) {
                $table->renameColumn('deadline', 'due_date');
            }

            // Drop columns only if they exist
            $columns = ['is_active', 'max_score', 'allow_late_submission', 'late_submission_penalty', 'visibility'];
            $existingColumns = collect($columns)->filter(fn($col) => Schema::hasColumn('assignments', $col))->toArray();
            
            if (!empty($existingColumns)) {
                $table->dropColumn($existingColumns);
            }
        });
    }
};
