...
<?php $__env->startSection('header', 'Daftar Tugas'); ?>
...
<?php $__env->startSection('content'); ?>
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="hidden md:flex items-center space-x-2">
                    <div class="bg-white/15 rounded-lg px-3 py-2 backdrop-blur-sm flex items-center">
                        ...
                        <span class="ml-2 bg-green-500 text-white text-sm px-2 py-0.5 rounded-md"><?php echo e($completedCount ?? 0); ?></span>
                    </div>
                    <div class="bg-white/15 rounded-lg px-3 py-2 backdrop-blur-sm flex items-center">
                        <span class="text-sm font-medium">Tugas tertunda: </span>
                        <span class="ml-2 bg-yellow-500 text-white text-sm px-2 py-0.5 rounded-md"><?php echo e($pendingCount ?? 0); ?></span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 mb-6 border border-gray-100/80">
        <form action="<?php echo e(route('siswa.assignments.index')); ?>" method="GET" class="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="flex flex-1 items-center w-full">
                <div class="relative rounded-md flex-1">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <i class="fas fa-search text-gray-400"></i>
                    </div>
                    <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari tugas..." 
    ...
    <div class="grid grid-cols-1 gap-6 mb-6">
        <?php $__empty_1 = true; $__currentLoopData = $assignments ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $assignment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100/80 hover:shadow-md transition-all duration-300 transform hover:-translate-y-1 group">
                ...
                <div class="p-1 <?php echo e($assignment->isExpired() && $assignment->submissions->isEmpty() ? 'bg-red-500' : ($assignment->submissions->isNotEmpty() ? 'bg-green-500' : 'bg-yellow-500')); ?>"></div>
                ...
                <div class="p-5">
                    <div class="flex flex-col md:flex-row justify-between">
                        <div class="flex-1">
                            <div class="flex items-center mb-3">
                                <span class="inline-flex items-center justify-center h-10 w-10 rounded-lg 
                                    <?php echo e($assignment->isExpired() && $assignment->submissions->isEmpty() ? 'bg-red-100 text-red-600' : 
                                      ($assignment->submissions->isNotEmpty() ? 'bg-green-100 text-green-600' : 'bg-yellow-100 text-yellow-600')); ?>">
                                    <i class="fas fa-<?php echo e($assignment->isExpired() && $assignment->submissions->isEmpty() ? 'exclamation-circle' : 
                                      ($assignment->submissions->isNotEmpty() ? 'check-circle' : 'clock')); ?> text-xl"></i>
                                </span>
                                <div class="ml-3">
                                    <h3 class="text-lg font-medium text-gray-800"><?php echo e($assignment->title); ?></h3>                                    <p class="text-sm text-gray-500 flex items-center">
                            ...
                            <div class="pl-12 pb-3">
                                ...
                                <div class="flex flex-wrap gap-3 text-sm">
                                    ...
                                    <!-- Submission Status -->
                                    <div class="<?php echo e($assignment->submissions->isNotEmpty() ? 'bg-green-100 text-green-800' : ($assignment->isExpired() ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800')); ?> px-3 py-1 rounded-full flex items-center">
                                        <i class="fas fa-<?php echo e($assignment->submissions->isNotEmpty() ? 'check' : ($assignment->isExpired() ? 'times' : 'clock')); ?> mr-1"></i>
                                        <span><?php echo e($assignment->submissions->isNotEmpty() ? 'Sudah Dikumpulkan' : ($assignment->isExpired() ? 'Belum Dikumpulkan (Terlambat)' : 'Belum Dikumpulkan')); ?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-3 md:mt-0 md:ml-4 flex flex-col items-center justify-center">
                            <?php if($assignment->submissions->isNotEmpty()): ?>
                                <div class="text-center">
                                    ...
                                    <div class="bg-green-50 rounded-lg px-4 py-2 text-center mb-3">
                                        <div class="text-sm text-green-800"><?php echo e($assignment->submissions->first()->created_at->format('d M Y, H:i')); ?></div>
                                    </div>
                                </div>
                            <?php elseif($assignment->isExpired()): ?>
                                <div class="text-center">
                            ...
                            <?php else: ?>
                                <div class="text-center">
                                    <span class="countdown bg-yellow-50 px-3 py-2 rounded-lg border border-yellow-100 flex flex-col items-center" data-deadline="<?php echo e($assignment->deadline); ?>">
                            ...
                            <!-- Action Button -->
                            <a href="<?php echo e(route('siswa.assignments.show', $assignment->id)); ?>" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-5 rounded-lg transition-all duration-300 flex items-center justify-center min-w-[120px] focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-opacity-50">
                                <?php if($assignment->submissions->isNotEmpty()): ?>
    ...
    <?php if(empty($assignments) || count($assignments) === 0): ?>
        <div class="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-md">
            ...
            <div class="mt-2">
                <a href="<?php echo e(route('siswa.assignments.index')); ?>" class="text-sm text-blue-600 hover:underline">
    ...
    <!-- Pagination -->
    <?php if(method_exists($assignments, 'hasPages') && $assignments->hasPages()): ?>
    ...
    <div class="mt-6 bg-blue-50 rounded-lg p-4 border border-blue-100">
        ...
        <div class="mt-3">
            <a href="<?php echo e(route('siswa.material.index')); ?>" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors">
...
<?php echo $__env->make('layouts.dashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\SEMUA TENTANG KULIAH\SEMESTER 4\PA2\IMPLEMENTASI NEW\sman1-girsip\resources\views/siswa/assignments/index.blade.php ENDPATH**/ ?>