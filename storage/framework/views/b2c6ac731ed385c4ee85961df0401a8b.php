...
<?php $__env->startSection('title', 'Daftar Pengumpulan Tugas'); ?>

<?php $__env->startSection('header', 'Daftar Pengumpulan Tugas'); ?>
...
<div class="container px-4 py-6 mx-auto">
                ...
                <div class="mb-4 md:mb-0">
                    <div class="flex items-center mb-2">
                        <span class="inline-flex items-center px-3 py-1 mr-2 text-xs font-semibold text-blue-800 bg-blue-100 border border-blue-200 rounded-full">
                            <i class="mr-1 fas fa-book"></i> <?php echo e($assignment->subject->name); ?>

                        </span>
                    </div>
                    <h2 class="text-2xl font-bold"><?php echo e($assignment->title); ?></h2>
                    <p class="mt-1 text-blue-100">
                        <i class="mr-1 fas fa-calendar-alt"></i> 
                        Deadline: <?php echo e($assignment->deadline ? $assignment->deadline->format('d M Y, H:i') : 'Tidak ada'); ?>

                    </p>
                </div>
                <div class="flex space-x-2">
                    <a href="<?php echo e(route('guru.assignments.show', $assignment)); ?>" class="flex items-center px-4 py-2 transition-colors rounded-lg shadow-sm bg-white/10 hover:bg-white/20 backdrop-blur-sm">
                        <i class="mr-2 fas fa-eye"></i> Detail Tugas
                    </a>
                    <a href="<?php echo e(route('guru.assignments.index')); ?>" class="flex items-center px-4 py-2 transition-colors rounded-lg shadow-sm bg-white/10 hover:bg-white/20 backdrop-blur-sm">
        ...
        <div class="p-4 bg-white border border-gray-100 shadow-sm rounded-xl">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <div class="text-xl font-bold text-gray-800"><?php echo e($submissions->total()); ?></div>
                </div>
            </div>
        </div>
        ...
        <div class="p-4 bg-white border border-gray-100 shadow-sm rounded-xl">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <div class="text-xl font-bold text-gray-800"><?php echo e($submissions->filter(function($submission) { return !is_null($submission->score); })->count()); ?></div>
                </div>
            </div>
        </div>
    ...
    <div class="p-4 mb-6 bg-white border border-gray-100 shadow-sm rounded-xl">
        <div class="flex flex-col items-center justify-between gap-4 sm:flex-row">            <form action="<?php echo e(route('guru.assignments.submissions.index', $assignment)); ?>" method="GET" class="flex flex-col items-center flex-1 gap-4 sm:flex-row">
                <div class="flex-1">
                ...
                <div class="flex items-center space-x-2">
                    <select name="status" class="border-gray-300 rounded-lg focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        ...
                        <option value="pending" <?php echo e(request('status') == 'pending' ? 'selected' : ''); ?>>Belum Dinilai</option>
                    </select>
            ...
            <div class="flex items-center mt-4 sm:mt-0">
                <a href="<?php echo e(route('guru.assignments.batch-grade', $assignment)); ?>" class="flex items-center px-4 py-2 font-medium text-white transition-colors bg-green-600 rounded-lg hover:bg-green-700">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php $__empty_1 = true; $__currentLoopData = $submissions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $submission): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex items-center justify-center w-10 h-10 text-gray-500 bg-gray-200 rounded-full">
                                        <?php echo e(strtoupper(substr($submission->student->name ?? 'U', 0, 1))); ?>

                                    </div>
                                    <div class="ml-3">
                                        <div class="text-sm font-medium text-gray-900"><?php echo e($submission->student->name ?? 'Unknown'); ?></div>
                                        <div class="text-xs text-gray-500"><?php echo e($submission->student->email ?? '-'); ?></div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-8 w-8 rounded-md <?php echo e($submission->file_color ?? 'bg-gray-500'); ?> flex items-center justify-center text-white">
                                        <i class="fas fa-<?php echo e($submission->file_icon ?? 'file'); ?>"></i>
                                    </div>
                                    <div class="ml-2">
                                        <div class="max-w-xs text-sm font-medium text-gray-900 truncate"><?php echo e($submission->file_name ?? basename($submission->file_path)); ?></div>
                                        <div class="text-xs text-gray-500"><?php echo e($submission->file_size ?? '-'); ?></div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900"><?php echo e($submission->created_at->format('d M Y')); ?></div>
                                <div class="text-xs text-gray-500"><?php echo e($submission->created_at->format('H:i')); ?></div>
                                <?php if($submission->created_at->gt($assignment->deadline)): ?>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php if($submission->score !== null): ?>
                                    <div class="text-sm font-medium text-gray-900"><?php echo e($submission->score); ?></div>
                                <?php else: ?>
                            ...
                            <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">                                <div class="flex items-center justify-end space-x-2">
                                    <a href="<?php echo e(route('guru.assignments.submissions.download', [$assignment->id, $submission->id])); ?>" class="px-2 py-1 text-blue-600 rounded hover:text-blue-900 bg-blue-50 hover:bg-blue-100">
                                        <i class="fas fa-download"></i>
                                    </a>
                                    <button type="button" class="px-2 py-1 text-green-600 rounded hover:text-green-900 bg-green-50 hover:bg-green-100 grade-btn" data-submission="<?php echo e($submission->id); ?>">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        gradingForm.action = `<?php echo e(route('guru.assignments.submissions.grade', ['assignment' => ':assignment', 'submission' => ':submission'])); ?>`
...
<?php echo $__env->make('layouts.dashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\SEMUA TENTANG KULIAH\SEMESTER 4\PA2\IMPLEMENTASI NEW\sman1-girsip\resources\views/guru/submissions/index.blade.php ENDPATH**/ ?>